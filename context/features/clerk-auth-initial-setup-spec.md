# Clerk Auth Initial setup

## Overview

This is the initial setup for Clerk in our app, we have already installed the package and now we need to hook it up to be able to signup.
...
This setup is linked to the Clerk application `app_3I8d8VZAxeysNzGqD1AOOV6MVfe`. Always
pass `--app app_3I8d8VZAxeysNzGqD1AOOV6MVfe` to `clerk init` so the project links to
the right Clerk app.

If this is an existing project, run:

```bash
clerk init --app app_3I8d8VZAxeysNzGqD1AOOV6MVfe
```
...
```bash
clerk init --framework <framework> --pm <package-manager> --app app_3I8d8VZAxeysNzGqD1AOOV6MVfe
```
...
`clerk init` currently has full scaffolding for Next.js
...
- https://clerk.com/docs/cli https://clerk.com/docs/llms.txt
